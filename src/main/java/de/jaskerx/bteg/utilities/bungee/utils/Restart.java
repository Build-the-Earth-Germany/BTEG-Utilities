...
public class Restart {
	...
	final Map<String, ServerInfo> servers = new TreeMap<>();
	final Map<String, ServerInfo> restartedServers = new TreeMap<>();
	boolean timerRuns = false;
	int counter;
	Timer timer;
	final boolean whenEmpty;
	private static boolean doProxyRestart = true;
	final ProxiedPlayer player;
	
	
	
	public Restart(String[] serversArgs, int delay, ProxiedPlayer p, boolean whenEmpty) {
		...
		if(p != null) {
			...
			servers.forEach((k, v) -> p.sendMessage(new ComponentBuilder(" §9" + k).create()));
		}
	...
	public void restart() {
		...
		servers.forEach((k, v) -> {
			if(!restartedServers.containsKey(k) && (ProxyServer.getInstance().getServers().containsValue(v) && (!whenEmpty || v.getPlayers().size() == 0)) || (k.equals("Proxy-1") && (doProxyRestart || !whenEmpty))) {
				ProxyServer.getInstance().getPluginManager().dispatchCommand(ProxyServer.getInstance().getConsole(), "cloud service " + k + (k.equals("Proxy-1") ? " stop" : " restart"));
